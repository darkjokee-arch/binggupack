...
v1.16 strangler Phase2: 본문 정본(byte-identical 이관). scripts/binggu_semantic_clean.py 는
  이 모듈을 re-export 하는 thin wrapper 다. _structural_keep 의 폴백 구조 휴리스틱만 scripts/
  sys.path anchor 를 repo 상대로 재계산(binggu_harvest bare-name lazy 의존 해소 · try/except graceful).
"""
...
def _structural_keep(text):
    ...
    정본 이관 후 binggu_harvest 는 아직 scripts/ 내부(bare-name)라 scripts/ 를 sys.path 에
    anchor 한다(repo 상대 계산 — pack 모듈 위치와 무관). 실패 시 try/except 로 보존(무손실)."""
    s = str(text or "").strip()
    if not s:
