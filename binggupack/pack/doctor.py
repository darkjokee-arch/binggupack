...
CLI: python -m ... / python scripts/openbinggu_doctor.py --selftest
...
strangler Phase2: 본문 정본은 이 패키지 모듈(binggupack.pack.doctor). scripts/openbinggu_doctor.py
는 sys.path 부트스트랩 후 정본을 re-export 하는 thin shim 이다(공개 심볼 identity 유지).
의존 3종(binggu_platform / openbinggu_label_kind_map / openbinggu_public_tree_scan)은 전부
패키지로 이관 완료 — 여기서는 패키지 정본에서 직접 import 한다. 하위 selftest 는 subprocess 로
경로(_HERE=<repo>/scripts) 를 지정해 실행하므로 위치 무관.
"""
import sys
import os
import re
...
# sys.path 가 의존하므로 기존 값과 동일하게 패키지 위치에서 역산해 유지한다.
_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # repo 루트(작업본)
_HERE = os.path.join(_ROOT, "scripts")  # 하위 selftest 스크립트 디렉터리(subprocess 경로 기준)

for _p in (_ROOT, _HERE):
    if _p and _p not in sys.path:
        sys.path.insert(0, _p)   # binggupack 패키지 + scripts/ 형제 import 경로

# 의존 3종 이관 완료 → 패키지 정본 직접 import.
from binggupack.workspace import platform as _plat  # noqa: E402
from binggupack.classifier.label_kind_map import classify_label_kind, KO2EN  # noqa: E402
