...
v1.16 strangler Phase2: 본 파일이 정본 모듈(binggupack.pack.knowledge_graph). scripts/binggu_knowledge_graph.py
는 공개 심볼 동일 thin wrapper 다. 미이관 형제 의존(binggu_local_collect line44 lazy·
binggu_branch_explorer __main__ lazy)은 정본 모듈이 scripts/ 를 sys.path 에 부트스트랩해 bare-name
으로 해소한다(recall 선례). 순수 transform·네트워크 0.
"""
import os
import sys
import json
...
# 미이관 형제(binggu_local_collect·binggu_branch_explorer)를 bare-name 으로 해소하기 위해
# scripts/ 를 sys.path 에 부트스트랩(recall 선례 — 정본 모듈이 scripts/ 경로를 자체 확보).
_HERE = os.path.dirname(os.path.abspath(__file__))                 # <repo>/binggupack/pack
