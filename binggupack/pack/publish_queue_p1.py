...
QueueError · IllegalTransition · ALLOWED_TRANSITIONS · TERMINAL)은 이 모듈로 byte-identical 이관됐다.
상태머신·hash 3중·fail-closed 판정은 1바이트도 변하지 않았다. top-level `import binggu_platform`
(MIGRATED shim · 미이관 bare-name) 을 위해 scripts/ 를 sys.path 에 얹는다(원본이 자기 위치(scripts/)를
...
# top-level `import binggu_platform`(MIGRATED shim · 미이관 bare-name) 해소 — 원본이 자기 위치(scripts/)를
# 얹던 것을 패키지 위치에서 scripts/ 로 재계산해 동일 효과. shim 경유/직접 import 양쪽 모두 안전.
