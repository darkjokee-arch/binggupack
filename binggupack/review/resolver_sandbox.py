...
strangler phase3: 순수 정본(ReviewResolver · _selftest · main)이 이 모듈로 byte-identical
이관됐고, scripts/openbinggu_review_resolver_sandbox.py 는 공개 심볼 동일한 thin wrapper 다.
dep(evaluate_confirm)은 정식 패키지 import(binggupack.safety.confirmed_governance — migrated)로
재배선됐다. sandbox·apply HOLD 불변은 1바이트도 변하지 않았다.
"""
import sys

from binggupack.safety.confirmed_governance import evaluate_confirm
