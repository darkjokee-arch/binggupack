...
v1.11.0 strangler phase3: 핵심 로직을 scripts/openbinggu_reviewed_plan_preview.py 에서
이 모듈로 이관했다. scripts 파일은 backward-compatible thin wrapper(sys.path bootstrap +
전체 심볼 re-export + __main__ CLI)로 유지되며 공개 심볼/동작은 byte-identical 하다(기능 변경 0).
...
BASE = Path(__file__).resolve().parent.parent.parent   # binggupack/review/ → binggupack → <repo>
