...
v1.11.0 save-gate 라인 S1: storage resolver 핵심 로직을 scripts/binggu_platform.py 에서
이 모듈로 이관했다. scripts 파일은 backward-compatible thin wrapper(sys.path bootstrap +
전체 심볼 re-export + __main__ platform_summary)로 유지되며 공개 심볼/동작/경로 계산은
byte-identical 하다(기능 변경 0). 순수 함수(write 0)·binggupack 외부 의존 0.
