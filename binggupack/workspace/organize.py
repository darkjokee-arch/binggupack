# -*- coding: utf-8 -*-
"""binggu_workspace_organize — 항목 W: 클라우드 workspace 정리(비파괴) 분석 + 정리안 산출.
...
v1.11.0 save-gate 라인: 본문 정본을 scripts/binggu_workspace_organize.py 에서 이 모듈로
이관했다. scripts 파일은 backward-compatible thin wrapper(sys.path bootstrap + 전체 심볼
re-export)로 유지되며 공개 심볼/동작/detector 로직은 byte-identical 하다(기능 변경 0).
"""
import argparse
import hashlib
import json
