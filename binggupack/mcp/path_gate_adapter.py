...
# -*- coding: utf-8 -*-
"""
OpenBinggu MCP path gate adapter (정본 in-package, 트랙 C strangler).

목적:
- MCP/local 도구의 모든 path 입력이 binggupack.safety.path_safety.classify_path 를 통과하게 한다.
- 도구 실행 "직전" 재검사로 TOCTOU 잔여를 줄인다.
- gate 실패(BLOCK) 시 실제 도구 함수를 호출하지 않는다.
- raw 경로값은 출력하지 않는다 → path_id / reason_code / count 만.

정본 이관(v1.11.x): 로직은 여기(binggupack/mcp/path_gate_adapter.py)가 정본이고
scripts/openbinggu_mcp_path_gate_adapter.py 는 공개 심볼을 재노출하는 thin shim 이다.
내부 참조는 scripts 우회 없이 binggupack.safety.path_safety 로 직결(순환 해소).
...
from binggupack.safety.path_safety import classify_path
