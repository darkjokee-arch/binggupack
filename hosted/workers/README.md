...
## 불변 원칙
read-only 5 tool만 · toy entry는 synthetic 한정 · real entry는 게이트 전건 통과 데이터만(레포에 실 데이터 0) · candidate-first(승격/병합 금지 룰 동봉) · fail-closed 누출 스캔 · Workers Logs/observability 끄기 유지(URL 토큰 기록 방지) · OAuth/write 미구현.
