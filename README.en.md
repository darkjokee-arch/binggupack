...
**Core (pip) vs Bridge (remote relay).** The local CLI, stdio MCP, ledger, recall and explain all work offline from `pip install` alone (Core). A separate remote relay (the hosted Cloudflare Worker) lets you approve memories from your phone / web / ChatGPT and pull them home (Bridge) — but **your local machine is always the source of truth**; the relay only forwards (it writes nothing to the ledger).

## 🛡 Safety model

- **No unapproved commits.** What becomes **active** memory always passes an explicit human approval. Approval is grounded in an **out-of-band anchor a model cannot reproduce** — your keyboard `SAVE n` (recorded by a UserPromptSubmit hook) or CLI TTY input. An agent that merely reproduces the confirm string (`SAVE 1`, `PAIR …`, `DEPRECATE …`) does **not** get human approval; MCP write paths are **fail-closed** without a real anchor.
