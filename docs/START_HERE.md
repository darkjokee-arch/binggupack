...
이 핵심 기능 위에 몇 가지 보조 기능도 있습니다.

| 기능 | 쉬운 뜻 |
|---|---|
| `preflight` | 작업 전에 관련 기억과 과거 실수 패턴을 먼저 보여줍니다 |
| `capture` hook | 원하면 Claude Code 발화에서 저장 후보만 모읍니다. 자동 저장은 아닙니다 |
| `pair` / `trust` | 내 말과 AI 의견을 따로 남기고, 나중에 판단 적중을 비교합니다 |
| `trace` | 떠올린 기억이 실제로 도움됐는지 표시합니다 |
| `hosted inbox` | 다른 기기에서 저장 의도만 잠깐 받아두고, 내 PC에서 고른 것만 가져옵니다 |
| MCP 서버 | Claude Code 같은 도구에서 빙구팩을 연결해 쓸 수 있습니다 |
