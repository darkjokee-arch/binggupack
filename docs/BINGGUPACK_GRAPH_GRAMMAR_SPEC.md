...
- **노드**: 단어 노드 금지 · 모든 정보를 노드로 만들지 않음 · **핵심 문장 노드 중심** ·
  기본 5종 canonical = **문서(Document) / 증거(Evidence) / 개념(Concept) / 상태(State) / 판단(Claim)** · 세부 노드는 핵심 문장에 종속
- **semantic_subtype(보조층)**: canonical 5종 **위에 얹는** 성격 태그(교훈 / 결정 / 선호 / 설계결정 / 버그패턴 / 사실 등). canonical node type으로 **승격 금지(hard fail)** — 노드 속성 필드로만 기록하며 저장·엣지 정체성에 관여하지 않음.
- **엣지**: 동사형 typed edge ("A는 B 판단의 근거가 된다"). node-to-node 근거 edge는 `supports_judgment` 기본 화이트리스트로 검증, 미등록 술어는 보류.
