...
> 규격 조항 ↔ BingguPack 실제 심볼. 이 문서는 Spec 이 **현재 구현을 정확히 서술**함을 보증하기 위한 추적표다. 기준 커밋: `9f5cac9` (main). 라인 번호는 해당 시점 기준이며 개정 시 갱신한다.
...
| preview 표시·영속 | `binggu.py` : `cmd_preview` (~848) |
...
| preview_id | `binggu.py` : `_preview_id` (~843) = `sha256(raw)[:8]` |
| SAVE n hook | `hooks/binggu_save_gate_hook.py` (~28) |
| 승인 3모드 판정 | `binggu.py` : `_resolve_human_ctx` (~917) |
...
| 로컬 pull / drain | `binggu.py` : `cmd_hosted` · `binggu_hosted_inbox.fetch_to_staging` |
