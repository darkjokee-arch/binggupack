...
# BingguPack 제품 방향 — 2트랙 재정의 (DIRECTION DOC, DESIGN ONLY)
...
- 상위: `BINGGUPACK_BUSINESS_SCOPE_DECISION_BRIEF.md`(internal design doc — not included in public repo)(4선택지 brief) · `BINGGUPACK_P0_BUNDLE_STATUS_UNDECIDED_HOLD.md`(internal design doc — not included in public repo) · `BINGGUPACK_PACK_APPROVAL_THREE_PATH_DESIGN.md`(internal design doc — not included in public repo).
...
| **우리 시스템/운영자의 OpenCrab store 자동 write/apply/ingest** | **HOLD** | `BINGGUPACK_PRODUCTION_HOLD_BOUNDARY.md`(internal design doc — not included in public repo) 조건 1~7 미충족. ※ OpenCrab은 가입자가 자기 pack을 자기 의지로 올리는 곳 — **사용자 주도 자발 업로드는 별개**(동일 fail-closed gate 적용 시 1차 배포 흐름 포함 가능, [FIRST_RELEASE_GITHUB_MCP_DESIGN](BINGGUPACK_FIRST_RELEASE_GITHUB_MCP_DESIGN.md) §2-4) |
