...
| capture→save 커밋 어댑터 | ✅ v1.3.1 — `commit_selected`(미리보기 후보를 사람 confirm 문구로 기존 `save_selected` 3중 게이트에 위임). **어댑터가 게이트를 생성하지 않고 confirm을 인자로 전달만** → actor=auto·confirm 누락/불일치·preview 미확인 전부 BLOCK, 자동저장 구조적 불가 유지. `scripts/binggu_capture_to_save.py`(13/13 selftest) |
| AGI memory capture (opt-in) | ✅ `python binggu.py init [--agi-memory]`이 **현재 repo/workspace scope에 자동 후보 수집을 기본 ON**으로 만듭니다(clone 직후엔 OFF — init 해야 동작). 전역 수집은 `--global` 명시해야만. `binggu.py capture {status,pause,resume,preview,uninstall}`로 제어. hook=`hooks/binggu_capture_hook.py`(기본 OFF 플래그 + scope 2중 게이트 · stdin 방어 · stdout 침묵). **자동 저장 아님**(candidate-only, ledger/active/confirmed write 0) — 저장은 preview→`SAVE n` 게이트만. selftest: persist 16/16 · profile 9/9 · hook 8/8 · CLI 21/21. 설치/scope/롤백: `docs/BINGGUPACK_CAPTURE_HOOK_SETUP.md` |
