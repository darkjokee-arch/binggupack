...
- **Latest release: v1.8.0** — <https://github.com/darkjokee-arch/binggupack/releases/tag/v1.8.0>
- **v1.8.0 핵심 — 똑똑한 "뜻 분류"가 자동으로 켜집니다.**
  - 대화에서 고른 문장을 **뜻으로 5종 분류**합니다(문서·증거·개념·상태·판단). 예전엔 문장 끝 표현만 보고 대부분 "판단"으로 뭉뚱그렸는데, 이제 **AI가 문장의 뜻을 읽어** 제대로 가립니다.
  - **한 번 깔면 자동**: AI 두뇌(Ollama + bge-m3 모델)를 한 번 설치하거나 폰/claude.ai로 쓰면 빙구팩이 **알아서 감지해 켭니다**. 안 깔아도 예전 방식으로 잘 동작하고, 저장 자체는 항상 됩니다. 끄려면 `BINGGU_SEMANTIC_OFF=1`.
  - **첫 설치 안내**: `binggu init` 하면 점검표가 떠서 "무엇이 켜졌고, 무엇을 더 깔면 뭐가 생기는지" 한눈에 보여줍니다(자동 설치는 안 하고, 설치 명령만 알려줍니다).
  - **안전**: AI는 도장 **제안**만 하고 저장 여부엔 관여하지 않습니다. 개인정보·비밀은 미리 걸러내고, 저장은 항상 `SAVE n` 사람 확인을 거칩니다(자동 저장 없음).
  - **OpenCrab 연동**(외부 지식그래프 도구): 내 데이터를 ZIP으로 만들어 OpenCrab에 넣는 **로컬 역인제스트**를 지원합니다. 단 이 기능은 **OpenCrab Expert 플랜에서만** 쓸 수 있습니다(약관상 Free·Pro 플랜은 외부 도구에서 OpenCrab으로 넣는 역방향 인제스트가 불가). Cloud 업로드는 보류(HOLD) 상태입니다.
...
2. **미리보기** — "빙구팩 저장해" 또는 `binggu capture preview`로 모인 후보를 확인합니다.
3. **사람 승인 저장** — `SAVE n`을 직접 타이핑해야만 저장됩니다. 그 외(자동·번호 없는 저장)는 전부 막힙니다.
4. **증거 기반 그래프** — 저장한 문장을 종류(5종)로 묶고 서로 관계로 잇되, **모든 연결에는 원문 근거가 붙습니다**(근거 없는 연결은 만들지 않음).
5. **여러 기기에서 보기** — 원본은 내 PC에만 두고, 폰·웹에서는 PC가 검증해 만든 **읽기 전용 사본**을 봅니다. 클라우드를 원본으로 쓰는 양방향 동기화는 보류(HOLD)입니다.
