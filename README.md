...
| 자동 후보 수집 hook support (opt-in) | ✅ 배포물 — `hooks/binggu_capture_hook.py`(UserPromptSubmit 발화 수집 · Stop 미리보기 건수). **기본 OFF**(`capture_enabled` 플래그) + scope 게이트(repo 화이트리스트 · 타 프로젝트 deny, fail-closed). **자동 저장 아님**(candidate-only, ledger/active/confirmed write 0) — 저장은 preview→`SAVE n` 게이트만. 본인 `settings.json` 등록 필요(opt-in). 영속 버퍼 `scripts/binggu_capture_persist.py`(14/14) + hook(8/8 selftest). 설치/롤백: `docs/BINGGUPACK_CAPTURE_HOOK_SETUP.md` |
