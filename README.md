...
1. 작업을 시작하거나 *"빙구팩 저장해"*로 마무리할 때, 빙구팩이 **지난번과 달라진 게 있는지** 확인해요.
2. 새로 저장된 게 있으면 → **민감정보를 먼저 걸러내고**(T3) → **바뀐 부분만** 추려요(델타).
3. 그걸 **AI(Claude)가 오픈크랩 팩에 대신 올려줘요.** *(자동 프로그램이 몰래 올리는 게 아니라, 안전을 위해 AI가 한 번 확인하며 올립니다.)*
4. 올리고 나면 "여기까지 올렸다"는 기준점을 갱신해서, **다음엔 또 새로 바뀐 것만** 올려요.
